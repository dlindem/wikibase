...
pyzot = zotero.Zotero(int(config_private.zotero_group_nr),'group',config_private.zotero_api_key) # Zotero LexBib group
linked_done = {}
legacy_qid = None
...
def zotero_export(infile=None):
	...
	done_items = []
	outfilename = infile.replace('.json', '_iwb_import_data.jsonl')
	...
	with open('data/linkattachmentmappings.jsonl', encoding="utf-8") as jsonl_file:
	...
	for item in data:
		...
		# 			# 	print('***ERROR: event not existing in iwb: '+eventcode)
		# 			# 	sys.exit()
		...
		# 				else:
		# 					print('Will check if this exists already on iwb; waiting for SPARQL...')
		# 					# check if container already exists (TBD)
		# 					query ="""
		# 							select ?item ?itemLabel where
		# 							{?item ldp:P44|ldp:P111 <"""+container+"""> ; rdfs:label ?itemLabel . filter(lang(?itemLabel)="en")
		# 							 }"""
		# 					# sparqlresults = sparql.query('https://wikibase.inguma.eus/query/sparql',query)
		# 					sparqlresults = iwbi.wbi_helpers.execute_sparql_query(query=query, prefix=iwbi.sparql_prefixes)['results']['bindings']
		# 					print('Got '+str(len(sparqlresults))+' results.')
		# 					#go through sparqlresults
		# 					v3container = None
		# 					for row in sparqlresults:
		# 						if row['item']['value'].startswith("https://wikibase.inguma.eus/entity/Q"):
		# 							v3container = row['item']['value'].replace("https://wikibase.inguma.eus/entity/","")
		...
		# 							v3container = iwb.newitemwithlabel("Q12","en",contlabel+voliss+contyear)
		# 							print('New container item is: '+str(v3container)+" "+contlabel+voliss+contyear)
		# 							print('Will write container entity data.')
		# 							iwb.itemclaim(v3container,"P5","Q16")
		# 							if contissn:
		# 								iwb.stringclaim(v3container,"P20",contissn)
		# 							if "volume" in item:
		# 								iwb.stringclaim(v3container,"P22",item['volume'])
		# 							if "issue" in item:
		# 								iwb.stringclaim(v3container,"P23",item['issue'])
		# 							iwb.stringclaim(v3container,"P111",container)
		# 							iwb.stringclaim(v3container,"P97",contlabel+voliss+contyear)
		#
		...
		# 	# 	# check if this location is already in iwb, if it doesn't exist, create it
		...
		# 	# 	else:
		# 	# 		print("This is a place unknown to iwb, will be created: "+wppagetitle)
		# 	# 		placeqid = iwb.newitemwithlabel("Q9", "en", wppagetitle)
		# 	# 		wdstatement = iwb.stringclaim(placeqid, "P2", wdid)
		# 	# 		wpstatement = iwb.stringclaim(placeqid, "P66", wppage)
		# 	# 		iwb.save_wdmapping({"iwbid": placeqid, "wdid": wdid})
		# 	# 		iwb.wdids[placeqid] = wdid
	...
	print(str(json.dumps(iwb_data)))
	# with open(infile.replace('.json', '_iwb_import_data.json'), 'w', encoding="utf-8") as json_file: # path to result JSON file
	# 	json.dump(iwb_data, json_file, indent=2)
	print("\n=============================================\nFinished. "+infile.replace('.json', '_iwb_import_data.jsonl')+". Finished.")
...
with open('data/ikergazte-doi-wikibase.csv') as csvfile:
	doilist = csv.DictReader(csvfile)
	doi_qid = {}
	for mapping in doilist:
		doi_qid[mapping['doi']] = mapping['wikibase_item'].replace(config_private.wb_entity_prefix,'')


def define_uri(item):
	# get iwb v2 legacy Qid, if any
	global linked_done
	global doi_qid
	bibItemQid = None
	if 'archive_location' in item:
		qidsearch = re.match(r'https://wikibase.inguma.eus/entity/(Q\d+)', item['archive_location'])
		if qidsearch:
			bibItemQid = qidsearch.group(1)
	if 'DOI' in item:
		if item['DOI'] in doi_qid:
			bibItemQid = doi_qid[item['DOI']]

	if not bibItemQid:	# set up new item
		print('No Qid found for "'+item['title']+'", will create new item.')
		bibItemQid = iwb.newitemwithlabel([], "en", item['title'])

	# # check if this zotId exists on iwb
	...
	#
	# 	iwb.logging.warning('Item mapping has disappeared from zotero data: ',zotitemid,qid)
	# else:
	...
	if bibItemQid and bibItemQid not in linked_done:
		...
		if attempts < 5:
			...
			if re.match(r'https?://wikibase.inguma.eus/entity/(Q\d+)', zot_qid):
				bibItemQid = re.match(r'https?://wikibase.inguma.eus/entity/(Q\d+)', zot_qid).group(1)
				legacy_qid = None
				# return bibItemQid
		...
		while attempts < 5:
			...
			json={"archiveLocation":"https://wikibase.inguma.eus/entity/"+bibItemQid,"version":version})

			if "204" in str(r):
				print('Successfully patched zotero item '+zotitemid+': '+bibItemQid)
				# with open(config_private.datafolder+'zoteroapi/iwbqid2zotero.csv', 'a', encoding="utf-8") as logfile:
		...
		if not att_presence:
			...
			"url": "https://wikibase.inguma.eus/entity/"+bibItemQid,
			"note": '<p>See this item as linked data at <a href="https://wikibase.inguma.eus/wiki/Item:'+bibItemQid+'">https://wikibase.inguma.eus/entity/'+bibItemQid+'</a>',
			"tags": [],
			"collections": [],
			"relations": {},
			"contentType": "",
			...
			if "200" in str(r):
				...
				try:
					...
					with open('data/linkattachmentmappings.jsonl', 'a', encoding="utf-8") as jsonl_file:
...
def wikibase_import(infile=None):
	...
	if not infile:
		...
		print('Please select post-processed Zotero export JSON to be imported to wikibase.inguma.eus.')
		#time.sleep(2)
		Tk().withdraw()
		infile = askopenfilename()
	...
	with open('data/logs/errorlog_'+infilename+'_'+time.strftime("%Y%m%d-%H%M%S")+'.log', 'w') as errorlog:
		index = 0
		edits = 0
		rep = 0
		...
		while index < totalrows:
			...
			if index >= 0: #start item in infile (change if process had been aborted without finishing)
				...
				iwbitem = iwbi.wbi.item.get(entity_id=lexBibID)
				print('Successfully got access to item '+iwbitem.id)
				# if 'lexBibClass' in item and item['lexBibClass'].startswith("Q"):
				# 	classStatement = iwb.updateclaim(lexBibID,"P5",item['lexBibClass'],"item")
				if len(item['creatorvals']) > 0:
					...
					# existingcreators = iwb.getclaims(lexBibID,creatortype)[1]
					try:
						existingcreators = iwbitem.claims.get(creatortype)
					except:
						existingcreators = []
					...
					for creatorstatement in item['creatorvals']:
						...
						if creatorstatement['value'] == False and "qualifiers" in creatorstatement: # the typical zotero literal output from zotexport.py
							...
							# 	creatorstatement = {'type':'item','prop_nr':creatortype,'value':False}
								# statement = iwb.updateclaim(lexBibID,triple['property'],creatorliteral,"novalue")

						# else:
						# 	statement = iwb.updateclaim(lexBibID,triple['property'],triple['value'],triple['datatype'])
						...
						if write_creator:
							...
							itemdata['statements'].append(creatorstatement)
								#iwb.setqualifier(lexBibID,triple['property'],statement, qualitriple['property'], qualitriple['value'], qualitriple['datatype'])
				langlabel = None
				englabel = None
				for statement in item['statements']:
					...
					# 			statements.app = iwb.updateclaim(lexBibID,triple['property'],sourceliteral,"novalue")
