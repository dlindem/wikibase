...
def update_group(groupname, rewrite=False): # if rewrite=True is passed, existing wikibase items are checked and re-written
	...
	for entry in group:
		...
		# productions: special for ikergazte
		if groupname == "productions":
			...
			if doi not in ikergazte:
				print('*** Fatal error: DOI not found.')
			else:
				# start and endpage
				pagessearch = re.search('(\d+)[_\-](\d+)', ikergazte[doi]['PDF'])
				if pagessearch:
					group[entry]['firstPage'] = pagessearch.group(1)
					group[entry]['lastPage'] = pagessearch.group(2)
					print('Pages',group[entry]['firstPage'],'to',group[entry]['lastPage'])
				# url
				group[entry]['url'] = ikergazte[doi]['PDF']
				print('PDF',group[entry]['url'])
