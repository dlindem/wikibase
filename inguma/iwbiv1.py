...
def itemwrite(iwbitem, statements, clear=False): # statements = {'append':[],'replace':[]}
	...
	if len(statements['replace']) > 0:
		iwbitem.claims.add(statements['replace'], action_if_exists=ActionIfExists.REPLACE_ALL)
	if len(statements['append']) > 0:
		iwbitem.claims.add(statements['append'], action_if_exists=ActionIfExists.APPEND_OR_REPLACE)
	d = False
	while d == False:
		try:
