...
def qualiP50():
	print('\nWill get LCR with P50-links without P126-year-of-pub qualifier, and years of the dists linked to them; same for disttype/bibtype.')

	query = """
	select ?lcr ?dist_statement ?dist (YEAR(?pubdate) as ?year) ?disttype ?bibtype where
	{?lcr lp:P50 ?dist_statement. ?dist_statement lps:P50 ?dist .
	...
	for item in bindings:
		...
		if lwb.setqualifier(lcr, 'P50', statement, 'P126', year, 'string'):
		...
		if disttype:
			if lwb.setqualifier(lcr, 'P50', statement, 'P91', disttype, 'item'):
				print('Success writing disttype for item ' + str(count))

def lcrfromdicts():
	mapping_cache = {}

	print('\nWill get dictionary distributions with no existing P50 link pointing to them.')

	query = """
	select ?uri (str(?label) as ?title) ?lcr ?codist ?codistlcr where
	{?uri ldp:P5 lwb:Q12; rdfs:label ?label. filter(lang(?label)="en") # Q12: Dictionary Distribution
	 filter not exists { ?lcr ldp:P50 ?uri .}
	 # OPTIONAL { ?uri ldp:P180 ?codist . optional {?codistlcr ldp:P50 ?codist.}}
	...
	for item in bindings:
		...
		if 'lcr' in item:
			...
			print('Found P50 link for this dictionary distribution. LCR is ' + lcrqid)
			mapping_cache[distrqid] = lcrqid
		else:
			print('No P50 link for ' + distrqid, title + ' dictionary distribution.')
			if 'codist' in item:
				...
				if 'codistlcr' in item:
					...
					codistlcr = lwbi.itemwrite({'qid': codistlcr,
												'statements': [{'prop_nr': 'P50', 'type': 'item', 'value': distrqid}]})
				...
				lcrqid = lwbi.itemwrite({'qid': False,
										 ...
										 'descriptions': [{'lang': 'eu', 'value': 'a lexical-conceptual resource'}],
										 'statements': [{'prop_nr': 'P5', 'type': 'item', 'value': 'Q4'},
														{'prop_nr': 'P50', 'type': 'item', 'value': distrqid}]})
				mapping_cache[distrqid] = lcrqid

	print('Finished. Written P50-links:\n' + str(mapping_cache))
...
def distdescriptions():
	...
	{?dist ldp:P5 lwb:Q12; ldp:P16 ?pubdate. filter not exists {?dist schema:description ?desc.}}
	 """
	...
	for item in bindings:
		...
		year = item['year']['value']
		if lwb.setdescription(dist, 'eu', f"LCR distribution ({year})"):
...
def lcrwikilist():
	...
	(year(?date) as ?creationdate) where {
	 ?lcr ldp:P5 lwb:Q4; ldp:P183 ?period .
	  ...
	  ?lcr ldp:P50 ?dist. ?dist ldp:P15 ?distdate; ldp:P16 ?distzot.
	  ?lcr ldp:P181 ?date.
