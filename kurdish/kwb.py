...
# 	global wdilogin
# 	global kwbEngine
# 	mediawiki_api_url = "https://kurdish.wikibase.cloud/w/api.php" # <- change to applicable wikibase
# 	sparql_endpoint_url = "https://kurdish.wikibase.cloud/query/sparql"  # <- change to applicable wikibase
# 	wdilogin = wdi_login.WDLogin(config_private.wb_bot_user, config_private.wb_bot_pwd, mediawiki_api_url=mediawiki_api_url)
# 	kwbEngine = wdi_core.WDItemEngine.wikibase_item_engine_factory(mediawiki_api_url, sparql_endpoint_url)
# 	print('Logged into WDI. (old kwb.py)...')
# 	return True
# wdisetup = None # If WDI is needed, login is done.

# LexBib wikibase OAuth for mwclient

site = mwclient.Site('kurdish.wikibase.cloud')
def get_token():
	global site
	...
	while True:
		try:
			...
			print('kwb login via mwclient raised error: '+str(ex))
			time.sleep(60)
	# get token
...
# Loads known kwbqid-lexbibUri mappings and kwbqid-Wikidataqid mappins from jsonl-files
def load_legacyID():
	legacyID = {}
	try:
		with open('data/legacymappings.jsonl', encoding="utf-8") as jsonl_file:
			...
			for mapping in mappings:
				...
				if mapping != "":
					try:
						...
						#print(mapping)
						legacyID[mappingjson['legacyID']] = mappingjson['kwbid']
					except Exception as ex:
...
def load_wdmappings():
	wdids = {}
	try:
		with open('data/kwb_wd.jsonl', encoding="utf-8") as f:
			...
			for mapping in mappings:
				...
				if mapping != "":
					try:
						mappingjson = json.loads(mapping)
						#print(mapping)
						wdids[mappingjson['kwbid']] = mappingjson['wdid']
					except Exception as ex:
						print('Found unparsable mapping json in kwb_wd.jsonl line ['+str(count)+']: '+mapping)
						print(str(ex))
						pass
	except Exception as ex:
	...
	print('Known kwb-WD item mappings loaded.')
	return wdids
...
# 	wpplaces = {}
# 	try:
# 		with open('data/mappings/wppage_kwbplace.jsonl', encoding="utf-8") as f:
# 			mappings = f.read().split('\n')
# 			count = 0
# 			for mapping in mappings:
# 				count += 1
...
# 						mappingjson = json.loads(mapping)
# 						#print(mapping)
# 						wpplaces[mappingjson['wppage']] = mappingjson['kwbid']
# 					except Exception as ex:
# 						print('Found unparsable mapping json in wppage_kwbplace.jsonl line ['+str(count)+']: '+mapping)
# 						print(str(ex))
# 						pass
# 	except Exception as ex:
...
def save_legacyID(legid,kwbid):
	with open('data/legacymappings.jsonl', 'a', encoding="utf-8") as jsonl_file:
		jsonline = {"legacyID":legid,"kwbid":kwbid}
		jsonl_file.write(json.dumps(jsonline)+'\n')
		global legacyID
		legacyID[legid] = kwbid

# Adds a new kwbqid-wdqid mapping to wdmappings.jsonl mapping file
def save_wdmapping(mapping): # example {"kwbid": "P32", "wdid": "P220"}
	with open('data/kwb_wd.jsonl', 'a', encoding="utf-8") as jsonl_file:
		jsonl_file.write(json.dumps(mapping)+'\n')
	global wdids
	wdids[mapping['kwbid']] = mapping['wdid']


# Get equivalent kwb item qidnum from wikidata Qid
def wdid2kwbid(wdid):
	print('Will try to find kwbqid for '+wdid+'...')
	global wdids
	# Try to find kwbqid from known mappings
	for key, value in wdids.items():
		if wdid == value:
			print('Found kwbqid in wdids known mappings: '+key)
			return key
...
# creates a new item
def newitemwithlabel(kwbclasses, labellang, label): # kwbclass: object of 'instance of' (P5)
	global token
	global legacyID
	if isinstance(kwbclasses, str) == True: # if a single value is passed as string, not as list
		kwbclasses = [kwbclasses]
	...
	while (not done):
		...
		for kwbclass in kwbclasses:
			done = False
			while (not done):
				claim = {"entity-type":"item","numeric-id":int(kwbclass.replace("Q",""))}
				...
				try:
					...
					if classclaim['success'] == 1:
						...
						print('Instance-of-claim creation for '+qid+': success. Class is '+kwbclass)
						#time.sleep(1)
				except:
...
#token = get_token()
# def getidfromlegid(kwbclasses, legid, onlyknown=False): # kwbclass: object of 'instance of' (P5), legid = value of (P1), pointing to data.lexbib.org legacy item id
# 	global token
# 	global legacyID
# 	if isinstance(kwbclasses, str) == True: # if a single value is passed as string, not as list
# 		kwbclasses = [kwbclasses]
...
# 	query = """PREFIX kwb: <http://data.lexbib.org/entity/>
# 	PREFIX owl: <http://www.w3.org/2002/07/owl#>
# 	select (REPLACE(STR(?redirect),".*Q","Q") AS ?redirect_qid) where {
# 	kwb:"""+legid+""" owl:sameAs ?redirect.}"""
...
#create string (or url or externalid) claim
	...
	while count < 5:
		...
		try:
			...
			if 'Invalid CSRF token.' in str(ex):
				...
				#get redirect target
				url = "https://kurdish.wikibase.cloud/query/sparql?format=json&query=PREFIX%20kwb%3A%20%3Chttp%3A%2F%2Flexbib.elex.is%2Fentity%2F%3E%0APREFIX%20ldp%3A%20%3Chttp%3A%2F%2Flexbib.elex.is%2Fprop%2Fdirect%2F%3E%0APREFIX%20lp%3A%20%3Chttp%3A%2F%2Flexbib.elex.is%2Fprop%2F%3E%0APREFIX%20lps%3A%20%3Chttp%3A%2F%2Flexbib.elex.is%2Fprop%2Fstatement%2F%3E%0APREFIX%20lpq%3A%20%3Chttp%3A%2F%2Flexbib.elex.is%2Fprop%2Fqualifier%2F%3E%0A%0Aselect%20%28strafter%28str%28%3Fredirect%29%2C%22http%3A%2F%2Flexbib.elex.is%2Fentity%2F%22%29%20as%20%3Frqid%29%20where%0A%7Bkwb%3A"+s+"%20owl%3AsameAs%20%3Fredirect.%7D%0A%20%20%0A"
				subdone = False
				while (not subdone):
					try:
						r = requests.get(url)
	...
	while count < 5:
		...
		try:
			...
			if 'Invalid CSRF token.' in str(ex):
				...
				with open ('data/logs/mergecandidates.log', 'a', encoding='utf-8') as mergecandfile:
...
def getclaims(s, p):
	...
	# https://kurdish.wikibase.cloud/wiki/Special:ApiSandbox#action=wbgetclaims&format=json&entity=Q14680
...
def newsense(lid, glosses, nodupcheck = None):
	...
	while done < 1:
		try:
			sensecreation = site.post('wbladdsense', token=token, format="json", lexemeId=lid, bot=1, data=json.dumps(data))
		except Exception as ex:
			if 'Invalid CSRF token.' in str(ex):
				print('Wait a sec. Must get a new CSRF token...')
				token = get_token()
		...
		if sensecreation['success'] == 1:
			#print(str(formcreation))
			senseid = sensecreation['sense']['id']
			print('Sense creation for '+lid+': success: '+senseid)
			# if wdform_id:
...
def updateclaim(s, p, o, dtype): # for novalue: o="novalue", dtype="novalue"
	...
	if dtype == "time":
		...
		# 	try:
		# 		item = kwbEngine(wd_item_id=s, data=data)
		# 	except Exception as ex:
	...
	elif dtype == "string" or dtype == "url" or dtype == "externalid":
		value = '"'+o.replace('"', '\\"')+'"'
	...
	if o not in foundobjs and value not in foundobjs: # must create new statement
		...
		print('*** Unknown error in kwb.updateclaim function, parameters were:',s, p, str(o), dtype)
		sys.exit()
...
def setclaimvalue(guid, value, dtype):
	...
	if dtype == "item" or dtype =="wikibase-entityid":
		...
		print('This item type is still not implemented in "kwb.setclaimvalue": '+dtype)
		sys.exit()
	while True:
		try:
			results = site.post('wbsetclaimvalue', token=token, claim=guid, snaktype="value", value=value)
...
def load_propmapping():
	...
	PREFIX lno: <https://kurdish.wikibase.cloud/prop/novalue/>
	PREFIX kwb: <https://kurdish.wikibase.cloud/entity/>
	PREFIX ldp: <https://kurdish.wikibase.cloud/prop/direct/>
	PREFIX lp: <https://kurdish.wikibase.cloud/prop/>
	PREFIX lps: <https://kurdish.wikibase.cloud/prop/statement/>
	PREFIX lpq: <https://kurdish.wikibase.cloud/prop/qualifier/>
	PREFIX lpr: <https://kurdish.wikibase.cloud/prop/reference/>
	...
	where {
	  ...
	  BIND (xsd:integer(REPLACE(str(?lexbib_prop), "https://kurdish.wikibase.cloud/entity/P", "")) as ?order )
	...
	sparqlresults = sparql.query('https://kurdish.wikibase.cloud/query/sparql',query)
	print('Got properties from LexBib v3 SPARQL.')
	#go through sparqlresults
	propmapping = {}
	for row in sparqlresults:
		sparqlitem = sparql.unpack_row(row, convert=None, convert_type={})
		pid = sparqlitem[1].replace("https://kurdish.wikibase.cloud/entity/","")
		propmapping[pid] = {}
		if sparqlitem[3]:
...
def getchangeditems(kwb_class,since):
	...
	url = "https://kurdish.wikibase.cloud/query/sparql"
	...
	for row in sparqlresults:
		...
		"qid":item[0].replace("https://kurdish.wikibase.cloud/entity/",""),
		"zot":item[1]
		})
