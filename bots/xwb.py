...
def addsense(s, glossdict):
	global token
	print(f'Will create sense with this gloss data: {glossdict}')
	done = False
	count = 0
	...
	while count < 5:
		count += 1
		try:
			request = site.post('wbladdsense', lexemeId=s, data=json.dumps(glossdict), token=token, bot=1)
			if request['success'] == 1:
				print(f'Success creating Sense\n{request}')
				return True
		except Exception as ex:
			if 'Invalid CSRF token.' in str(ex):
				print('Wait a sec. Must get a new CSRF token...')
				...
				print('Addsense operation failed, will try again...\n'+str(ex))
				# logging.error('Description set operation '+s+' ('+lang+') '+val+' failed, will try again...', exc_info=True)
				time.sleep(4)
	# log.add
...
def setqualifier(qid, prop, claimid, qualiprop, qualio, dtype):
	global token
	print(f"GUID: {claimid}")
	guidfix_re = re.search(r'^([QLP]\d+)\-(.*)', claimid)
	if guidfix_re:
		claimid = guidfix_re.group(1) + '$' + guidfix_re.group(2)
	# guidfix = re.compile(r'^([QLP]\d+)\-')
	# claimid = re.sub(guidfix, r'\1$', claimid)
	print(f"good GUID: {claimid}")
