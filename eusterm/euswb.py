...
# 	global wdilogin
# 	global euswbEngine
# 	mediawiki_api_url = "https://eusterm.wikibase.cloud/w/api.php" # <- change to applicable wikibase
# 	sparql_endpoint_url = "https://eusterm.wikibase.cloud/query/sparql"  # <- change to applicable wikibase
# 	wdilogin = wdi_login.WDLogin(config_private.wb_bot_user, config_private.wb_bot_pwd, mediawiki_api_url=mediawiki_api_url)
# 	euswbEngine = wdi_core.WDItemEngine.wikibase_item_engine_factory(mediawiki_api_url, sparql_endpoint_url)
# 	print('Logged into WDI. (old euswb.py)...')
# 	return True
# wdisetup = None # If WDI is needed, login is done.

# LexBib wikibase OAuth for mwclient

site = mwclient.Site('eusterm.wikibase.cloud')
def get_token():
	global site

	# euswb login via mwclient
	while True:
		try:
			login = site.login(username=config_private.wb_bot_user, password=config_private.wb_bot_pwd)
			break
		except Exception as ex:
			print('euswb login via mwclient raised error: '+str(ex))
			time.sleep(60)
	# get token
	csrfquery = site.api('query', meta='tokens')
	token = csrfquery['query']['tokens']['csrftoken']
	print("Got fresh CSRF token for eusterm.wikibase.cloud.")
	return token
token = get_token()
...
def load_wdmappings():
	wdids = {}
	try:
		with open('data/wdmapping.csv', encoding="utf-8") as f:
			mappings = f.read().split('\n')
			for row in mappings:
				mapping = row.split('\t')
				if len(mapping) == 2:
					wdids[mapping[0]] = mapping[1]
	except Exception as ex:
		print ('Error: wdmappings file does not exist. Will start a new one.')
		print (str(ex))

	print('Known euswb-WD item mappings loaded.')
	return wdids
...
# 	wpplaces = {}
# 	try:
# 		with open('data/mappings/wppage_euswbplace.jsonl', encoding="utf-8") as f:
# 			mappings = f.read().split('\n')
# 			count = 0
...
# 						#print(mapping)
# 						wpplaces[mappingjson['wppage']] = mappingjson['euswbid']
# 					except Exception as ex:
# 						print('Found unparsable mapping json in wppage_euswbplace.jsonl line ['+str(count)+']: '+mapping)
# 						print(str(ex))
# 						pass
# 	except Exception as ex:
...
# Adds a new euswbqid-wdqid mapping to wdmappings.jsonl mapping file
def save_wdmapping(mapping): # example {"euswbid": "P32", "wdid": "P220"}
	with open('data/euswb_wd.jsonl', 'a', encoding="utf-8") as jsonl_file:
		jsonl_file.write(json.dumps(mapping)+'\n')
	global wdids
	wdids[mapping['euswbid']] = mapping['wdid']


# Get equivalent euswb item qidnum from wikidata Qid
def wdid2euswbid(wdid):
	print('Will try to find euswbqid for '+wdid+'...')
	global wdids
	# Try to find euswbqid from known mappings
	for key, value in wdids.items():
		if wdid == value:
			print('Found euswbqid in wdids known mappings: '+key)
			return key

# creates a new item
def newitemwithlabel(euswbclasses, labellang, label): # euswbclass: object of 'instance of' (P5)
	global token
	if isinstance(euswbclasses, str) == True: # if a single value is passed as string, not as list
		euswbclasses = [euswbclasses]
	...
	while (not done):
		...
		for euswbclass in euswbclasses:
			done = False
			while (not done):
				claim = {"entity-type":"item","numeric-id":int(euswbclass.replace("Q",""))}
				...
				try:
					...
					if classclaim['success'] == 1:
						...
						print('Instance-of-claim creation for '+qid+': success. Class is '+euswbclass)
						#time.sleep(1)
				except:
	...
	while count < 5:
		...
		try:
			...
			if 'Invalid CSRF token.' in str(ex):
				...
				#get redirect target
				url = "https://eusterm.wikibase.cloud/query/sparql?format=json&query=PREFIX%20euswb%3A%20%3Chttp%3A%2F%2Feusterm.wikibase.cloud%2Fentity%2F%3E%0APREFIX%20ldp%3A%20%3Chttp%3A%2F%2Feusterm.wikibase.cloud%2Fprop%2Fdirect%2F%3E%0APREFIX%20lp%3A%20%3Chttp%3A%2F%2Feusterm.wikibase.cloud%2Fprop%2F%3E%0APREFIX%20lps%3A%20%3Chttp%3A%2F%2Feusterm.wikibase.cloud%2Fprop%2Fstatement%2F%3E%0APREFIX%20lpq%3A%20%3Chttp%3A%2F%2Feusterm.wikibase.cloud%2Fprop%2Fqualifier%2F%3E%0A%0Aselect%20%28strafter%28str%28%3Fredirect%29%2C%22http%3A%2F%2Feusterm.wikibase.cloud%2Fentity%2F%22%29%20as%20%3Frqid%29%20where%0A%7Beuswb%3A"+s+"%20owl%3AsameAs%20%3Fredirect.%7D%0A%20%20%0A"
				subdone = False
				while (not subdone):
					try:
						r = requests.get(url)
...
def getclaims(s, p):
	...
	# https://eusterm.wikibase.cloud/wiki/Special:ApiSandbox#action=wbgetclaims&format=json&entity=Q14680
...
def updateclaim(s, p, o, dtype): # for novalue: o="novalue", dtype="novalue"
	...
	if dtype == "time":
		...
		# 	try:
		# 		item = euswbEngine(wd_item_id=s, data=data)
		# 	except Exception as ex:
...
def load_propmapping():
	...
	PREFIX lno: <https://eusterm.wikibase.cloud/prop/novalue/>
	PREFIX euswb: <https://eusterm.wikibase.cloud/entity/>
	PREFIX ldp: <https://eusterm.wikibase.cloud/prop/direct/>
	PREFIX lp: <https://eusterm.wikibase.cloud/prop/>
	PREFIX lps: <https://eusterm.wikibase.cloud/prop/statement/>
	PREFIX lpq: <https://eusterm.wikibase.cloud/prop/qualifier/>
	PREFIX lpr: <https://eusterm.wikibase.cloud/prop/reference/>
	...
	where {
	  ...
	  BIND (xsd:integer(REPLACE(str(?lexbib_prop), "https://eusterm.wikibase.cloud/entity/P", "")) as ?order )
	...
	sparqlresults = sparql.query('https://eusterm.wikibase.cloud/query/sparql',query)
	print('Got properties from LexBib v3 SPARQL.')
	#go through sparqlresults
	propmapping = {}
	for row in sparqlresults:
		sparqlitem = sparql.unpack_row(row, convert=None, convert_type={})
		pid = sparqlitem[1].replace("https://eusterm.wikibase.cloud/entity/","")
		propmapping[pid] = {}
		if sparqlitem[3]:
		...
		if sparqlitem[5]:
			propmapping[pid]['range'] = sparqlitem[5].replace("https://eusterm.wikibase.cloud/entity/","")
		print(str(propmapping))
	return propmapping

# get member items of a certain LexBib ontology class (string) changed since a certain date (string)
def getchangeditems(euswb_class,since):
	...
	{
	  ?bibitem ldp:P5 euswb:"""+euswb_class+""" ;
	           ldp:P16 ?zotero ;
	...
	url = "https://eusterm.wikibase.cloud/query/sparql"
	...
	for row in sparqlresults:
		...
		"qid":item[0].replace("https://eusterm.wikibase.cloud/entity/",""),
		"zot":item[1]
		})
